...
from src.jolly_brancher.issues import IssueStatus, IssueType, JiraClient, get_all_issues, get_issue

class TestIssueStatus(unittest.TestCase):
    def test_selectable_statuses(self):
        expected = [IssueStatus.TODO, IssueStatus.IN_PROGRESS, IssueStatus.BACKLOG, IssueStatus.NEW, IssueStatus.IN_REVIEW, IssueStatus.BLOCKED, IssueStatus.QA, IssueStatus.STAGED, IssueStatus.DONE]
...
class TestJiraClient(unittest.TestCase):
    ...
    def test_get_all_issues(self):
        self.client._JIRA.search_issues.return_value = MagicMock(total=1, issues=[MagicMock(key="JIRA-123", fields=MagicMock(summary="Test issue"))])
